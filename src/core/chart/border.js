 ...
 * @returns {string} SVG path data string.
 */
export function combinedBorderPath(polygons, style, roundingSize, maxAngleFactor, maxEdgeConsumption, gap) {
    style = style || "straight";

    if (style === "straight" && !gap) {
        return deduplicatedEdgePath(polygons);
    }

    return polygons.map(polygon => borderPath(polygon, style, roundingSize, maxAngleFactor, maxEdgeConsumption, gap)).join("");
}